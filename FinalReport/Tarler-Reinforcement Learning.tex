...
\usepackage{graphicx}
\shorttitle{MSDS696: Practicuum 2}
...
\For {episode = 1,M} 
	...
	\For {$t=1,T}$ 
		...
		\State Sample random minibatch of transactions $(\phi_{j}, a_{j}, r_{j}, \phi_{j+1})$ from $\mathcal{D}$
		\State Set \[y_{i} = 
		\left\{\begin{array}{lr}
...
We will first examine the agent, or the bipedal walker. The Bipedal Walker or $\mathbb{A}$ has two distinct atributes: the action space $\mathcal{A}$ and the observation space $\mathcal{O}$. In general, our neural network is a function, $D$ such that $D: \mathcal{O} \mapsto \mathcal{A}$.  

The observation space for the robot is rather large, that is $\vert \mathcal{O} \vert = 24$. That is, there are twenty-four different obsevations that the agent can make about itself (numbers 0-13) and about the environment using its Lidar (14-23).  
